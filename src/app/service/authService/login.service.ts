...
export class LoginService {
  ...
  public login(request: any) {
    return this.http.post("http://localhost:8092/login", request, { responseType: 'text' as 'json',observe:'response'})
  }
  ...
  public checkForToken(){
    ...
    if(token!=null){
      if(this.tokenExpired(token)){
        ...
        this.alert.apiSuccessMsgReload('Session Expired need to login again',4000)
        this.router.navigate(['home']).then(() => {
          window.location.reload();
